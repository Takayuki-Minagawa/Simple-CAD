...
2. 左パネルの **Story** でフロアを切り替え
3. ツールバーの描画ツール（Column / Beam / Wall / Slab / Dim / Text）で部材を追加
4. **Select** ツールでオブジェクトを選択 → 右パネルでプロパティを編集
5. **2D / 3D** ボタンで表示モードを切り替え
6. **Export** で SVG / PDF / DXF に出力
7. **AI** ボタンで LLM 連携（プロンプトコピー・JSON 貼り付け読み込み）
8. 🌙 / ☀️ ボタンでダーク / ライトモードを切替
9. **EN / JA** ボタンで表示言語を切替
10. **ヘルプ** ボタンで簡易マニュアルを表示
...
| DXF 取込 | 自作パーサー (LINE / LWPOLYLINE / CIRCLE / ARC / TEXT) |
